...
? What is the reason React and ReactDOM are separate packages?

 -> You create React elements very similar to how you create DOM elements, except React elements are very simple objects that describe the UI that you want to create. You take that UI, and you render it to the page using ReactDOM. The reason that these two things are separate is because React supports other platforms as well. You have React Native and React VR. You can use the core React to create React elements for any of these platforms. Then you use the platform-specific package to get those React elements into the environment where you're shipping them to, whether that's native or DOM.
