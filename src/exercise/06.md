...
? What is the difference between controlled and uncontrolled input?
   ...
   -> controlled is stored in a state variable and the onChange handler will call setState to keep the value up-to-date, while with the uncontrolled input, the input state is maintained by the browser
