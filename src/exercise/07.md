...
? Why do we need a unique `key` prop when rendering an array in React?

  -> to help React keep track of elements between updates, otherwise the order can get mixed up
  
We can see this in the extra credit demo. If we correctly use the key prop, the focus state and highlight are being maintained for the correct input, even when things get shuffled.
